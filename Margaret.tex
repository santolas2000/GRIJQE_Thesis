...
Those parameters are: the \textit{stopping power}, denoted by $-\diff E / \diff x$, and the \textit{momentum broadening coefficient}, denoted by $\hat{q}$.
...
%\textcolor{red}{Should these auxiliary calcs be moved to an appendix?} 
To prove the first equality of Eq. (\ref{eq:qHat_from_X}),
\begin{equation*}
...
For the purpose of obtaining $Y^\alpha$ from $X^{\alpha\beta}$, $T$ is taken to be the temperature of a thermalized QGP with an energy density equal to that of the glasma. That is, the temperature which would characterize the system achieved if the glasma were allowed to reach equilibrium without expanding. %\textcolor{red}{[Comments on validity?] - partly discussed in sec. 5 of Mrowczyncki\_2018}

From Eqs. (\ref{eq:collision_from_tensors_v0}) and (\ref{eq:Y_from_X}), the final expressions for determining $\hat{q}$ and $\diff E / \diff x$ from $X^{\alpha \beta}$ become:
\begin{subequations}
\begin{align}
...
\noindent where 
\begin{equation}
\mathbf{y} = \mathbf{x} - \mathbf{v} (t - t')
\end{equation}
is the probe's past position, under the eikonal approximation, where the probe is undeflected as it traverses the medium and its velocity is constant.

Thus, the $X^{\alpha \beta}$ component of the tensor is obtained by integrating, in time, over the full past trajectory of the probe, the correlator of the $\alpha$ and $\beta$ components of the Lorentz color force; the first at the ``present'' (time $t$, position $\mathbf{x}$), and the second at a past point of the probe's trajectory (time $t'$, position $\mathbf{y}$). In the eikonal approximation, where the probe is undeflected as it traverses the medium, its velocity is constant, and the probe's past position is:
\begin{equation}
	\mathbf{y} = \mathbf{x} - (t-t')\mathbf{v}
	\label{eq:past_position}
\end{equation}
...
Plugging Eq. (\ref{eq:lorentz_force_component}) into Eq. (\ref{eq:short_X}) allows unfolding the expression for $X^{\alpha \beta}$:
\begin{equation}
\label{eq:unfolded_X}
\begin{split}
	X^{\alpha \beta} (\textcolor{red}{t,\mathbf{x}}, \mathbf{v}) =  \frac{g^2}{2 N_c} \int_0^t \diff t'   
	 \bigg[ &  \left< E^\alpha_a(t, \mathbf{x} ) E^\beta_a(t', \mathbf{y} )\right> \\ 
	& + \varepsilon^{\beta \gamma \gamma'} v^\gamma \left< E^\alpha_a(t, \mathbf{x} ) B^{\gamma'}_a(t', \mathbf{y} )\right> \\
	& + \varepsilon^{\alpha \gamma \gamma'} v^\gamma \left< B^{\gamma'}_a(t, \mathbf{x} ) E^{\beta}_a(t', \mathbf{y} )\right> \\
	& + \varepsilon^{\alpha \gamma \gamma'} \varepsilon^{\beta \delta \delta'} v^\gamma v^\delta \left< B^{\gamma'}_a(t, \mathbf{x} ) B^{\delta'}_a(t', \mathbf{y} )\right> \bigg]
\end{split}
\end{equation}
