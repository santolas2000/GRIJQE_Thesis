...
\begin{figure}[h]
    \centering
    %\includegraphics[width=\textwidth]{Figures/gluon_recombination.png}
    \include{Diagrams/recombination}
...
The evolution equations of \gls{QCD} -- the \gls{DGLAP} equations and the \gls{BFKL} equations -- also predict the growth of gluon densities at low-$x$.

The \gls{DGLAP} differential equations describe how \glspl{PDF} should evolve with $\mu$ at fixed $x$.

The \gls{BFKL} differential equations, on the other hand, describe how gluon density in momentum space, $\phi$, should evolve with $x$ at fixed $\mu$. It is a linear equation, and, as introduced, it predicts
%an exponential growth of the gluon density with rapidity, Y = ln(1/x); equivalently, it predicts
a divergence of the gluon density as $x \to 0$; to be specific, it predicts an exponential growth of $\phi$ with rapidity, $y \equiv \ln(1/x)$. Nonetheless, the equation does not contain any terms reflecting interference between gluon emitters, as it assumes the hadron to be a dilute parton system. However, making the probability of emission of a new gluon dependent on the present gluon density by adding non-linear terms allows for saturation to be reached when gluon generation and gluon recombination balance out. These new non-linear terms have coefficients that are inverse energy scales, $(1/Q)^n$, which control when the new terms become comparable to the old linear terms, and thus correspond to saturation scales, $Q_s$. \cite{Albacete_2014} 
...
Fig. \ref{fig:HIC_lightcone} suggests the usefulness of a coordinate system where the axes lie along the nuclei's path in the $x^1Ox^0$ plane. In the Ligh-Cone coordinate system (see Appendix \ref{ap:Light_Cone}), 
\begin{subequations}
\begin{gather}
...
scattering applied to describe the aforementioned saturated gluons \cite{Gelis_2010}, which evade standard perturbation techniques. It is one of the approaches adopted to study initial state effects in \glspl{HIC}; that is, the influence of the properties of the system prior to the formation of the \gls{QGP}, and their impact on final-state observables. It is used, for example, in the modeling of the incoming nuclei and of the non-equilibrated medium that leads to the \gls{QGP}. In this regard, it has stood out due to its practical applicability and phenomenological success. \cite{Albacete_2014} 
...
First, the treatment of the saturated gluons as classical fields will be justified. The saturated gluon densities at low-$x$ discussed previously in Sections \ref{sec:nuclear_matter} and \ref{sec:gluon_saturation} imply gluon fields (also called color fields) of the order $A \sim 1/g$, which does not allow their treatment through standard perturbative techniques\footnote{For example, in calculating the matrix element $\mathcal{M}$ for a \gls{QCD} process using a series expansion in powers of $g$, all terms are proportional to $gA \sim \mathcal{O}(1)$, which does not allow truncating the series in order to estimate $\mathcal{M}$ -- all orders need to be summed.}. \cite{Albacete_2014} However, it opens the possibility of their treatment as classical fields.

Comparing the occupation number of the saturated gluon fields and the commutator of their creation and annihilation operators, $a$ and $a^\dagger$ \citep{Albacete_2014}
\begin{equation}
...
\\
\intertext{
where $0<x_i<1$ is the fraction of the nuclear longitudinal momentum carried by the $i$-th parton; it was further assumed that the boost applied was such that $x_i P \gg m_i$, with $m_i$ being the $i$-th parton's mass. In light-cone coordinates, where $a^+ = (a^0+a^1)/\sqrt{2}$, $a^- = (a^0-a^1)/\sqrt2$ and $a^2, a^3$ are unchanged (see Appendix \ref{ap:Light_Cone} for more details),}
...
The parton content of the nucleus is thus divided into the group of high $+$-momentum ($p^+ > \Lambda^+$) static color charges and the group of low $+$-momentum ($p^+ < \Lambda^+$) dynamic classical gluon fields which they generate. The static color charges may themselves be treated as classical \citep{Albacete_2014,McLerran_1994_classic_source}, and are expressed in the formalism as a classical color current four-vector, $J^\mu \equiv J^\mu_a T^a$. This current is eikonally \textcolor{red}{define} coupled to the classical gluon fields; that is, the static charges suffer negligible recoil from their interactions with the color fields, when compared to their longitudinal momentum. \cite{Gelis_2010} This is expressed by inserting the color current into the Lagrangian via the term $A_\mu^a J^\mu_a$. Since, now, the formalism does not include quantum quark fields, Eq.  (\ref{eq:QCD_Lagrangian_Indices}) becomes 
...
Seeing as $J^\mu$ describes partons traveling at near the speed of light in the $x^1$ direction, then, in light-cone coordinates, its only non-vanishing component is $J^+$ (Eq. (\ref{eq:Nonvanishing_current_+})). Additionally, it will directly depend on the number color charge distribution of the valence partons, $\rho (x) \equiv \rho_a(x) T^a$ (Eq. (\ref{eq:J_from_rho})).
\begin{gather}
	J^\mu = \delta^\mu_+ J^+ \label{eq:Nonvanishing_current_+} \\
	J^+(x^\mu) = g \rho (x^\mu) \label{eq:J_from_rho}
\end{gather}

From the previous discussion on the static nature of the color charges, $\rho$'s dependence on $x^+$ can be dropped. Furthermore, due to the first conclusion drawn from the uncertainty relations (Eqs. \ref{eq:Uncertainty_Relations}), the color charge distribution function describing the placement of the static valence partons is very thin in the $x^-$ direction. 
...
So far, the framework's dependence on the arbitrary choice of a $+$-momentum scale $\Lambda^+$ to separate low- and high-$x$ partons
% and its impact on the final value of an observable to be computed, $\left< \mathcal{O} \right>$, 
has been neglected. Indeed, once a $+$-momentum scale has been set, one neglects that certain low-$x$ partons sorted into the category of classical fields actually may act as static color sources for lower-$x$-still partons\footnote{In the language of Section \ref{sec:gluon_saturation}, even a low-$x$ gluon can serve as an emitter of lower-$x$ gluons.} \cite{Albacete_2014}. Thus, choosing a different $\Lambda^+$ would lead to different observable values. \textcolor{red}{(Next sentence needs re-writing and compartamentalizing)} When changing the value of the $+$-momentum scale from $\Lambda^+$ to ${\Lambda^+}'$, it is possible, however, to renormalize $\rho$, correlators of different $\rho$ or the functional probability distribution  $W$ in order to incorporate those quantum corrections into the \gls{CGC} effective theory and preserve the value of the observables, making them independent of $\Lambda^+$, while keeping the calculation procedure strictly classical \citep{Albacete_2014}. Choosing to incorporate these quantum corrections into the probability distribution $W$, which then becomes dependent on the $+$-momentum scale, $W_{\Lambda^+}$, its evolution with $\Lambda^+$ is ruled by the \gls{JIMWLK} equation (read ``Jim-Walk") \cite{Gelis_2010}.
...
In the present work, the Yang-Mills equations in the case of a single nucleus are initially solved in the Lorenz, or covariant, gauge, where $\partial_\mu A^\mu=0$, following the more recent solving of the full equations from \cite[Section 2.2]{Blaizot_2004}\footnote{Note that this paper's covariant derivative definition is $D_\mu=\partial_\mu -igA_\mu$. However, this does not affect the final result.}. The solution is then brought to light-cone gauge, where $A^+=0$, an axial gauge condition, following the procedure from \citep[Section 3A]{Carrington_2022}. \textcolor{blue}{(\citep{Kovner_1995}: "Using light cone gauge one has direct access to the gluon distribution functions of the parton model" -- I wasn't able to find a justification for this, in order to include it as a motivation to work in light-cone gauge. Is there any reference you know I could use?)} %\textcolor{red}{I was going to write something here. Maybe the residual gauge freedom?}
...
Once again requiring that the current and fields  \textcolor{red}{vanish in the far past}
