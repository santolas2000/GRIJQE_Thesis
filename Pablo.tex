...
\subsubsection{In the \gls{MV} model}
...
\end{equation}
which is solved in momentum space\footnote{The 2D inverse Fourier transform of any radial function $\tilde{f}(\mathbf{k}) = \tilde{f}(k)$ is its Hankel transform over $k$, divided by $2 \pi$. The Hankel transform of $1/(k^2 + m^2)$ is given in \cite{Gradshteyn:1943cpj}, integral 6.5324}
\begin{equation}
...
\end{equation}
where $K_0$ is the modified Bessel function of the second kind of degree $0$.
