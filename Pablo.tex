...
\begin{equation}
	\left< \rho_a(x^-,\mathbf{x}_\perp) \rho_b (y^-,\mathbf{y}_\perp) \right> =  \delta^{ab} \delta^2 (\mathbf{x}_\perp - \mathbf{y}_\perp) \delta (x^- - y^-) \mu_A^2 \textcolor{red}{\lambda(x^-, \mathbf{x}_\perp)}
...
Moving to the scenario of a HIC, where both nuclei are present, the recent solvings in \cite{Albacete_2019,Guerrero_Rodr_guez_2021} will be followed.
...
\subsection{The two-point gluon field correlator}
...
\end{subequations}
The simple forms of these contractions are the motivation for the separation of the $\delta^{ij}$ and $r^i r^j$ components of the corrrelator as it is done in Eq. (\ref{eq:general_correlator}). These contractions will be useful in Chapter \ref{ch:merge}.
...
\begin{equation}
	- \nabla^2_\perp \Phi(x^-, \mathbf{x}_\perp) = \rho (x^-,\mathbf{x}_\perp) \quad \Rightarrow \quad 
	\Phi(x^-,\mathbf{x}_\perp) = - g  \int \diff^2 \mathbf{y}_\perp G(\mathbf{x_\perp} - \mathbf{y_\perp}) \rho(x^-,\mathbf{y}_\perp)
\end{equation}

\textcolor{purple}{DIFFERENT NAME FOR THE GREEN'S FUNCTION}

The Green's function should obey the equation:
\begin{equation}
	-? \nabla_\perp^2 G (\mathbf{x}_\perp) = 
\end{equation}
-----------------------------------------------
