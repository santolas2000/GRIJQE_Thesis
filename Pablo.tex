...
A more recent calculation for the full equations can be found in \cite[Section 2.2]{Blaizot_2004}.

The Yang-Mills equations are initially solved in the Lorenz, or covariant, gauge, where $\partial_\mu A^\mu=0$. The solution is then brought to light-cone gauge, where $A^+=0$, an axial gauge condition. \textcolor{blue}{(\citep{Kovner_1995}: "one has direct access to the gluon distribution functions of the parton model")}. \textcolor{red}{I was going to write something here.}
...
A solution may be built by solving this system iteratively, for the fields and current expanded in terms of powers of the color charge density, $\rho$. The terms of order $(n)$ are obtained from those of lower orders by:
\begin{equation}
	\begin{cases}
...
Moving to the $n=2$ system, the right side of the top equation is once again $0$ since $A_{(1)}^\mu$ and $J_{(1)}^{\mu}$ only have a $+$-component. For the bottom equation, its commutator vanishes because $A_{(1)}^\mu$ only has a $+$-component which is independent of $x^+$. The system for $n=2$ is, then,
\begin{equation}
	\begin{cases}
...
Finally, in order to move to light-cone gauge, where $A_{\text{LC}}^{+}=0$, the appropriate gauge transformation matrix $U$ is found by solving the equation:
\begin{equation}
...
\begin{gather}
	U(x^-,\mathbf{x}_\perp) = U(x^-_0, \mathbf{x}_\perp) + \int_{x^-_0}^{x^-} \diff z A^+ (z, \mathbf{x}_\perp) U (z, \mathbf{x}_\perp) 	\\
	U(x^-,\mathbf{x}_\perp) = \lim_{n \to \infty}\left(\mathds{1} 
	+ \int_{x^0_-}^{x^-} \diff z_0 A (z_0)
	+ \int_{x^0_-}^{x^-} \diff z_0 A (z_0)\int_{x^0_-}^{z_0} \diff z_1 A (z_1) + ...  
	+ \int_{x^0_-}^{x^-} \diff z_0 A (z_0)\int_{x^0_-}^{z_0} \diff z_1 A (z_1) ... \int_{x^0_-}^{z_{n-1}} \diff z_n A (z_n)
	 \right) U(x_0^-,\mathbf{x}_\perp)
\end{gather}


where $x^-_0$ was chosen to be $-\infty$ and $U(x_0^-,\mathbf{x}_\perp)$ was chosen to be $\mathds{1}$, without loss of generality.

\textcolor{red}{TALK ABOUT CHARGE DENSITY IN THE COVARIANT GAUGE}
