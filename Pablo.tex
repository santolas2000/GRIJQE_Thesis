...
\end{equation}
where $D^\eta = J_0(k \tau)$  and $D^2=D^3 = -i \varepsilon^{ij} (k^j/k) J_1(k \tau)$; and  $C^{mn} = \delta^{mn}$ for $E^\eta$ and $B^i$ and $C^{mn} = \varepsilon^{mn}$ for $E^i$ and $B^\eta$.
...
\textcolor{red}{UNSURE} A general form for the correlator is:
\begin{equation}
...
\end{equation}
where $\mathbf{r} \equiv \mathbf{x}_\perp - \mathbf{y}_\perp$. 
