...
a divergence of the gluon density as $x \to 0$; more specifically, it predicts an exponential growth of $\phi$ with rapidity, $y \equiv \ln(1/x)$. The equations do not contain any terms reflecting interference between gluon emitters, as they assume the hadron to be a dilute parton system. However, making the probability of emission of a new gluon dependent on the present gluon density by adding non-linear terms to the equations allows for saturation to be reached when gluon generation and gluon recombination balance out. The saturation scale $Q_s$ thus arises as the transverse momentum scale below which the new non-linear terms become comparable in magnitude to the old linear terms, leading to saturation. \cite{Albacete_2014}

%These new non-linear terms have coefficients that are inverse powers of energy scales, $(1/Q)^n$; they control when the new terms become comparable to the old linear terms, and thus act as saturation scales, $Q_s$. \cite{Albacete_2014}
...
\begin{equation}
	\left< \rho_a(x^-,\mathbf{x}_\perp) \rho_b (y^-,\mathbf{y}_\perp) \right> =  \delta^{ab} \delta^2 (\mathbf{x}_\perp - \mathbf{y}_\perp) \delta (x^- - y^-) \rho(x^-, \mathbf{x}_\perp)
...
\begin{align}
	...
	=  \mathcal{P}^+ \exp  \Bigg[ig \int_{-\infty}^{x^+} \diff z ~  g \int \diff^2 \mathbf{y}_\perp \mathcal{G}(\mathbf{x_\perp} - \mathbf{y_\perp}) \tilde{\rho}_2(z,\mathbf{y}_\perp) \Bigg] \nonumber 
\end{align}
...
\begin{figure}
    \centering    \includegraphics[width=0.6\textwidth]{Figures/flux_tubes.png}
...
In order to find the expressions for the fields at $\tau \to 0$ ($\tilde{\beta}_0$ and $\tilde{\beta}^i_0$), the next step would be to use the boundary conditions of Eqs. (\ref{eq:boundary_conditions}), which would require first bringing the pre-collision potentials, $\alpha^i_1$ and $\alpha^i_2$, to the Coulomb gauge. Solving the Yang-Mills equations with numerically computed $\tilde{\beta}_0$ and $\tilde{\beta}^i_0$ in this way has been found to approximate the resolution of the full Yang-Mills equations quite well, except for very soft modes (very low $|\mathbf{k}_\perp|$). \cite{Guerrero_Rodr_guez_2021,Blaizot:2010kh} However, no analytical expression for the necessary transformation matrix $V(\mathbf{x}_\perp)$ has been found.
...
The two-point gluon field correlator, 
\begin{equation}
	\left< \alpha^i_{pa} (\mathbf{x}_\perp) \alpha^j_{qb} (\mathbf{y}_\perp) \right>
\end{equation}
will be a useful quantity for Chapter \ref{ch:merge}, and it is computable within the present formalism for the pre-collision potentials. Furthermore, the two-point gluon field correlator, when calculated in light-cone gauge, corresponds to the Weizsäcker-Williams gluon distribution, a \gls{PDF} of the gluon (multiplied by $x$). The correlator should be $0$ for gluon fields from different nuclei, $p \neq q$, as they are causally unrelated, and for different colors, $a \neq b$ \cite{Lappi:2017skr}.
...
In the ultraviolet limit ($r \to 0$), the fractions at the r.h.s. of Eqs. (\ref{eq:unregularized_MV_G_h}) approach $1$, and so does $mrK_1(mr)$; however, due to the $K_0 (mr)$ term, $G_{\text{MV}}$ diverges logarithmically, and so must be regularized. A logarithmic scaling of the unpolarized gluon distribution with distance is encouraging, as it seems analogous to the \gls{DGLAP} evolution equations' results. \cite{Guerrero_Rodr_guez_2021}
